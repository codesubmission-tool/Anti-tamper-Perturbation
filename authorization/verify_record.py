...
for noise_type,noise_args in [(None,0),('JPEG',70),('JPEG',50),('Resize',2),('Resize',4)]:
    ...
    if noise_type is None:
            bit_error_set['origin'] = np.array(bitwise_avg_err)
    else:
        bit_error_set[f'{noise_type}_{noise_args}'] = np.array(bitwise_avg_err)

name = target_dir.split('/')[-1]
import pickle
with open('../metrics/results'+f'/{name}_biterror','wb') as f:
    pickle.dump(bit_error_set,f)
